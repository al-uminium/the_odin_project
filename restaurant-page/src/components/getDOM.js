export const getDOM = function() {
    ...
    const addImage = (path, par, attr='',attrname='') => {
        let img = document.createElement("img")
        img.setAttribute("src", path)
        if (attr && attrname) {
            img.setAttribute(attr,attrname)
        }
        par.appendChild(img)
    }

    const addText = (node, str, anim='') => {
    ...
    return {createDiv, addChildren, addImage, clearText, addText, loremIpsum}
}